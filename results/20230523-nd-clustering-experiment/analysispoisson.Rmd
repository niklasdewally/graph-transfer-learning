---
title: "Poisson Clustering Results"
...
library(readr)
data <- read_csv("datapoisson.csv", col_types = cols(model = col_factor(levels = c("triangle", 
...
p <- ggplot(plotData%>% filter(`srctarget`=="target"),aes(x=`accuracy`,y=`label`,colour=`model`)) + 
  geom_boxplot() + 
  facet_grid(rows=vars(`src`),cols=vars(`target`)) +
  ggtitle("Model types by source and target graph type (poisson).")
p + theme_bw()
```
```{r echo=FALSE}
p <- ggplot(plotData%>% filter(`srctarget`=="source"),aes(x=`accuracy`,y=`label`,colour=`model`)) + 
  geom_boxplot() + 
  facet_grid(rows=vars(`src`),cols=vars(`target`)) +
  ggtitle("Model types by source and target graph type (poisson).")
p + theme_bw()
